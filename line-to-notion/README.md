...
- ラベルは「タイトル」「日時」「リンク」の3種類（全角・半角コロン両対応、順不同、リンクは省略可。これはLINEメッセージ上のラベルであり、Notion側の列名とは別物）
...
- 抽出した「タイトル」は実際の「予定」データベースの**「スポット名」**列に、「リンク」は**「リンク」**列に書き込まれる（実際の列名は下記「Notion側の事前準備」参照）
