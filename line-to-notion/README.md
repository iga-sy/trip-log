...
- ラベルは「タイトル」「日時」の2種類（全角・半角コロン両対応、順不同。これはLINEメッセージ上のラベルであり、Notion側の列名とは別物）
...
- 「予定」データベース側の「修」「美」「悠」「紗」の各列（コメント欄）や「旅行名」「場所」列はLINEからは書き込まれず、常に空欄で作成される。感想やコメントは、これまで通りNotion上でその予定を開いて各自が直接入力する
...
- 「投稿者」列（タイトル型）はLINEからは常に空のまま作成する（将来的に旅行名などの用途に使う想定のため）
